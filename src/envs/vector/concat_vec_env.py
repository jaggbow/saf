...
class ConcatVecEnv(gym.vector.VectorEnv):
    ...
    def reset(self, seed=None):
        ...
        if seed is not None:
            for i in range(len(self.vec_envs)):
                _res, _state = self.vec_envs[i].reset(seed=seed + i)
                _res_ls.append(_res)
                _res_states.append(_state)
        else:
            for vec_env in self.vec_envs:
                _res, _state = vec_env.reset(seed=None)
                _res_ls.append(_res)
                _res_states.append(_state)

        return self.concat_obs(_res_ls), self.concat_states(_res_states)
    ...
    def step(self, actions):
        ...
        for venv in self.vec_envs:
            ...
            idx += venv.num_envs
        observations, states, rewards, dones, infos = transpose(data)
