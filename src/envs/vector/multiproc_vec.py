"""
Adapted from:
https://github.com/Farama-Foundation/SuperSuit/blob/767823849865af32050fee5c6a7d2fa7d86c4973/supersuit/vector/multiproc_vec.py
"""
import copy
...
def write_states(vec_env, env_start_idx, shared_state, state):
    state = list(iterate(vec_env.state_space, state))
    for i in range(vec_env.num_envs):
        write_to_shared_memory(
            vec_env.state_space,
            env_start_idx + i,
...
def async_loop(vec_env_constr, inpt_p, pipe, shared_obs, shared_state, shared_rews, shared_dones):
    inpt_p.close()
    try:
        ...
        while True:
            ...
            if instr == "close":
                ...
                if name == "reset":
                    observations, states = vec_env.reset(seed=data)
                    write_observations(vec_env, env_start_idx, shared_obs, observations)
                    write_states(vec_env, env_start_idx, shared_state, states)
                    ...
                    observations, states, rewards, dones, infos = vec_env.step(actions)
                    write_observations(vec_env, env_start_idx, shared_obs, observations)
                    write_states(vec_env, env_start_idx, shared_state, states)
...
class ProcConcatVec(gym.vector.VectorEnv):
    def __init__(
        self, vec_env_constrs, observation_space, state_space, action_space, tot_num_envs, metadata
    ):
        ...
        self.shared_state = create_shared_memory(self.state_space, n=self.num_envs)
        ...
        self.states_buffers = read_from_shared_memory(
            self.state_space, self.shared_state, n=self.num_envs
        )
    ...
    def reset(self, seed=None):
        ...
        return numpy_deepcopy(self.observations_buffers), numpy_deepcopy(self.states_buffers)
    ...
    def step_wait(self):
        ...
        return (
            ...
            numpy_deepcopy(self.states_buffers),
            rewards.copy(),
            dones.copy(),
            copy.deepcopy(infos),
        )
