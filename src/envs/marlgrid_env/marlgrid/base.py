...
class MultiGridEnv(gym.Env):
    ...
    def action_space(self):
        return gym.spaces.Tuple([agent.action_space for agent in self.agents])

    @property
    def observation_space(self):
        return gym.spaces.Tuple([agent.observation_space for agent in self.agents])
...
class MultiGridEnv_coodinationNheterogeneity(gym.Env):
    def __init__(
        ...
        AllActions = list(
            itertools.permutations(list(range(7)))
        )  # it is import that the SAME order is used across episodes
...
class MultiGridEnv_PrisonBreakNheterogeneity(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        self._gen_grid(
            10, 10
        )  # manually make the size of room smaller to make game easier
        ...
        for shuffled_ix in iter_order:
            ...
            if agent.active:
                ...
                for a_idx in range(len(self.agents)):
                    ...
                    if (
                        abs(x2 - x1) <= Distance_range
                        and abs(y2 - y1) <= Distance_range
                    ):
                        N_agents_around += 1
    ...
    def step(self, actions):
        ...
        for shuffled_ix in iter_order:
            ...
            if agent.active:
                ...
                if (
                    agent.carrying is not None and isinstance(agent.carrying, Key)
                ) and (N_agents_around >= self.coordination_level):
                    allow_reward = True
                ...
                # Move forward
                    ...
                    if can_move:
                        ...
                        if (
                            hasattr(fwd_cell, "get_reward") and allow_reward
                        ):  # reward only allowed if carrying a key and supported by other agents
                ...
                # Pick up an object
                    ...
                    if (
                        hasattr(cur_cell, "get_reward") and allow_reward
                    ):  # reward only allowed if carrying a key and supported by other agents
                        rwd = cur_cell.get_reward(agent)
                        if bool(self.reward_decay):
                            rwd *= 1.0 - 0.9 * (self.step_count / self.max_steps)
...
class MultiGridEnv_coodinationNheterogeneityCompoundGoal(gym.Env):
    ...
    def is_valid(self, pos):
        ...
        if grid_obj is None:
            ...
            for adj_pos in adj_positions:
                ...
                if isinstance(adj_obj, EmptyGoal) or isinstance(
                    adj_obj, CompoundGoalTile
                ):
                    # print(f'Failed2')
                    return False
    ...
    def place_compound_obj(
        self, obj, top=(0, 0), size=None, reject_fn=None, max_tries=1e5
    ):
        max_tries = int(max(1, min(max_tries, 1e5)))
        top = (max(top[0], 0), max(top[1], 0))
        if size is None:
