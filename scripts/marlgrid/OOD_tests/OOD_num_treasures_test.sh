...
ExpName="OOD_num_treasures_test_"${env_name}"_"${N_agents}"_"${coordination}"_"${heterogeneity}"_"${policy}"-"${use_policy_pool}"-"${latent_kl}"_"${num_treasures}"_"${seed}
...
n_agents=${N_agents} \
env_steps=50 \
env.params.num_goals=${num_treasures} \
